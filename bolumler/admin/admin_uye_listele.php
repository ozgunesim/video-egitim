...
if(isset($_GET['sayfa']))	//sayfa numarasi aliniyor
	$sayfa = $_GET['sayfa'];
else
	$sayfa = 1;
...
<div class="table-responsive">
	...
	<tr><th>Kullanıcı Adı</th><th>E-Mail</th><th>Yetki</th><th>İşlem?</th></tr>
	<?php
	
	foreach ($dizi_uye as $satir) {												//uye listesi tablolanıyor
		$yetkiler[0] = "yok";
		...
		echo "<tr>";
			...
			<button type="button" <?php echo $disabled; ?> id="btn_uye_sil_<?php echo $satir['ID']; ?>" class="btn btn-primary btn-sm" data-toggle="modal" data-target="#mdl_uye_yetki_<?php echo $satir['ID']; ?>">
			 	Yetki Değiştir
			</button>
			<!-- Modal -->
			<div class="modal fade" id="mdl_uye_yetki_<?php echo $satir['ID']; ?>" tabindex="-1" role="dialog" aria-hidden="true">
			  <div class="modal-dialog modal-sm">
			    <div class="modal-content">
			      <div class="modal-header">
			        <button type="button" class="close" data-dismiss="modal" aria-label="Close"><span aria-hidden="true">&times;</span></button>
			        <h4 class="modal-title">Üye Yetkilendirme</h4>
			      </div>
			      <div class="modal-body">
				        Yetki Türünü Seçin<hr>
				        <div class="radio">
				        	<Label>
				        		<input type="radio" name="yetki_radio<?php echo $satir['ID']; ?>" onclick="yetki_sec(3);" \>Öğrenci
				        	</Label>
				        </div>
				        <div class="radio">
				        	<Label>
				        		<input type="radio" name="yetki_radio<?php echo $satir['ID']; ?>" onclick="yetki_sec(2);" \>Öğretmen
				        	</Label>
				        </div>
				        <div class="radio">
				        	<Label>
				        		<input type="radio" name="yetki_radio<?php echo $satir['ID']; ?>" onclick="yetki_sec(1);" \>Admin
				        	</Label>
				        </div>
			      ...
			      <div class="modal-footer">
			      	<button id='mdl_btn_uye_sil_" . $satir['ID'] . "' onclick="yetkilendir(<?php echo $satir['ID']; ?>);" $disabled class='btn btn-primary'> Yetkilendir</button>
			        <button type="button" class="btn btn-default" data-dismiss="modal">Vazgeç</button>
			      </div>
			    </div>
			  </div>
