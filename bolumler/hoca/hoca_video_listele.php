...
if(isset($_GET['sayfa']))	//sayfa numarası alınıyor
	$sayfa = $_GET['sayfa'];
else
	$sayfa = 1;
?>
...
<table class="table table-striped table-hover">
	...
	<?php
		$dizi = $db->hoca_videolari_listele($sayfa,$aralik,$_SESSION['uye_ID']);	//alınan sayfa numarasına gore videolar listeleniyor
		$sayac = 0;
		foreach ($dizi as $satir) {
			echo "<tr id='vid_satir_" . $satir['ID']. "'>";
				?>
...
<?php
$max_sayfa_araligi = 5;
$video_sayisi = $db->hoca_video_sayi_getir($_SESSION['uye_ID']);	//sayfa sayısı hesaplanıyor
